...
class CranSEO_AI {
    ...
    private function check_quota() {
        if (empty($this->license_key)) {
            return array(
                'within_quota' => false,
                'remaining' => 0,
                'limit' => 3, // Default basic plan credits
    ...
    /**
     * Update quota usage - now handled by API server during content generation
     */
    ...
    /**
     * Get upgrade message for when credits are depleted
     */
    public function get_upgrade_message() {
        $quota_info = $this->check_quota();
        $remaining = $quota_info['remaining'];
        $limit = $quota_info['limit'];
        $tier = $quota_info['license_tier'];
    ...
    public function generate_content($post_id, $content_type) {
        ...
        if (!$quota_info['has_credits']) {
            throw new Exception($this->get_quota_exceeded_message());
        }
    ...
    /**
     * Get consistent quota exceeded message
     */
    private function get_quota_exceeded_message() {
        return sprintf(
            __('You have used all your available credits. <a href="%s" target="_blank">Upgrade your plan</a> to generate more content.', 'cranseo'),
            $this->get_upgrade_url()
        );
    }
